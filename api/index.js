...
mongoose
  .connect(
    'mongodb+srv://lakshmigayathri:gayathri_v7@cluster0.bpgab.mongodb.net/',
  )
  .then(() => {
...
// });
app.post('/register', async (req, res) => {
  const { name, email, password, image, role } = req.body; // Include role in the destructuring

  // Create a new user with the role included
  const newUser = new User({ name, email, password, image, role });
  ...
  newUser
    ...
    .catch(error => {
      console.log('Error creating a user:', error); // Log the error for debugging
      res.status(500).json({ message: 'Error registering the user' });
    });
});
...
app.post('/login', async (req, res) => {
  try {
    ...
    res.status(200).json({ token, role: user.role }); // Return role in response
  } catch (error) {
